...
export async function processDecisionTreeAlerts(snapshot, notify) {
  const expired = await expireDecisionTreeAlerts();
  ...
  for (const rule of rules) {
    ...
    result.checked += 1;
    // Stored alerts currently persist numeric conditionKind/lowerPrice/upperPrice rules only.
    // Keep evaluation deterministic so scheduler rearm semantics remain stable.
