...
export async function requestAiJson({ messages, temperature = 0, maxTokens = 1200, task, fetchImpl = fetch } = {}) {
...
export async function requestAiChat({ messages, temperature = 0.3, maxTokens = 900, fetchImpl = fetch } = {}) {
  const result = await requestAiCompletion({
    messages,
    temperature,
    maxTokens,
