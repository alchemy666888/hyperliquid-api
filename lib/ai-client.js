...
const EMPTY_TEXT_RETRY_MAX_TOKENS = 4096;
...
function deepSeekChoiceMetadata(payload) {
...
async function requestDeepSeekCompletion({
  ...
  return {
    ...
    usage: payload.usage,
    provider: config.provider,
    ...deepSeekChoiceMetadata(payload),
  };
}
...
async function requestAiCompletion({
  ...
  try {
    ...
    if (controller?.signal?.aborted) {
      return { ok: false, error: `${label} request timed out after ${normalizedTimeoutMs}ms.`, provider: config.provider };
    }
...
function retryMaxTokensForEmptyText(maxTokens) {
  const current = Number.isFinite(maxTokens) && maxTokens > 0 ? maxTokens : 900;
  return Math.max(
    current + 1,
    Math.min(EMPTY_TEXT_RETRY_MAX_TOKENS, Math.max(current * 3, current + 1200)),
  );
}

function shouldRetryEmptyText(result) {
  return result?.provider === 'DEEPSEEK'
    && !result.text
    && result.finishReason === 'length';
}

function emptyTextError(result) {
  const label = providerLabel(result?.provider);
  if (result?.finishReason === 'length') {
    return `${label} response hit the max token limit before producing text.`;
  }
  if (result?.finishReason === 'content_filter') {
    return `${label} response was filtered and did not contain text.`;
  }
  if (result?.finishReason === 'tool_calls') {
    return `${label} requested tool calls instead of text.`;
  }
  if (result?.finishReason === 'insufficient_system_resource') {
    return `${label} response was interrupted by insufficient provider resources.`;
  }
  if (result?.hasReasoningContent) {
    return `${label} response contained reasoning but no final text.`;
  }
  return `${label} response did not contain text.`;
}

export async function requestAiJson({ messages, temperature = 0, maxTokens = 1200, task, fetchImpl = fetch, searchEnabled, timeoutMs } = {}) {
  ...
  if (!json || Array.isArray(json)) {
    return {
      ok: false,
      error: result.text
        ? `${providerLabel(result.provider)} response did not contain a JSON object.`
        : emptyTextError(result),
...
export async function requestAiChat({ messages, temperature = 0.3, maxTokens = 900, fetchImpl = fetch, searchEnabled, timeoutMs } = {}) {
  let result = await requestAiCompletion({
    messages,
    temperature,
    maxTokens,
    fetchImpl,
  ...
  if (shouldRetryEmptyText(result)) {
    const retryResult = await requestAiCompletion({
      messages,
      temperature,
      maxTokens: retryMaxTokensForEmptyText(maxTokens),
      fetchImpl,
      searchEnabled,
      timeoutMs,
    });
    if (!retryResult.ok) return retryResult;
    result = { ...retryResult, retriedEmptyText: true };
  }

  if (!result.text) {
    return {
      ok: false,
      error: emptyTextError(result),
      provider: result.provider,
      finishReason: result.finishReason,
    };
  }
  ...
  return {
    ...
    provider: result.provider,
    finishReason: result.finishReason,
    retriedEmptyText: Boolean(result.retriedEmptyText),
  };
}
