...
export async function commitStage(client, jobId, {
  ...
  if (!client) {
    const result = await withPlanJobsClient(dbClient => commitStage(dbClient, jobId, {
      outputColumn,
      output,
      nextStage,
      nextStatus,
    }));
    if (!result) throw new Error('PostgreSQL unavailable; could not commit plan stage.');
    return result;
  }
...
export async function markReplySent(client = getPool(), jobId) {
  if (!client) {
    return withPlanJobsClient(dbClient => markReplySent(dbClient, jobId));
  }
