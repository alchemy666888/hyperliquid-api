...
import { searchGoogleForContext } from './search.js';
...
const EXPLICIT_WEB_SEARCH_PATTERN = /\b(?:google|look\s*up|search|web search|find out)\b|谷歌|搜索|搜一下|查一下/i;
const WEB_SEARCH_TOPIC_PATTERN = /\b(?:announc(?:e|ed|ement|es)|breaking|ceo|current(?:ly)?|election|headline|headlines|latest|launch(?:ed|es)?|laws?|news|policy|president|prime minister|recent(?:ly)?|regulations?|release(?:d|s)?|results?|schedule|scores?|updates?|who won)\b|公告|宣布|当前|更新|结果|今日|近日|目前|赛程|实时|谁赢|新闻|消息|现在|最近|最新|总统|法规|监管|政策/i;
const MARKET_NEWS_PATTERN = /\b(?:announc(?:e|ed|ement|es)|breaking|earnings|etf|fed|headline|headlines|lawsuit|news|policy|regulations?|sec|tariff|upgrade|downgrade)\b|公告|宣布|新闻|消息|监管|政策|财报/i;
...
export function isWebSearchRelatedRequest(message, { marketRelatedRequest = false } = {}) {
  const text = String(message ?? '').trim();
  if (!text) return false;
  if (EXPLICIT_WEB_SEARCH_PATTERN.test(text)) return true;
  if (!WEB_SEARCH_TOPIC_PATTERN.test(text)) return false;

  return !marketRelatedRequest || MARKET_NEWS_PATTERN.test(text);
}
...
export async function answerStatelessAiChat({
  ...
  getSearch = searchGoogleForContext,
  deepSeekChat,
  ...
  const webSearchRelatedRequest = isWebSearchRelatedRequest(currentMessage, { marketRelatedRequest });
  const webSearchContext = webSearchRelatedRequest
    ? await getSearch({ query: currentMessage })
    : null;
    ...
    messages: [
      {
        ...
        content: [
          ...
          'For current events, news, recent announcements, changing public facts, schedules, or policies, use webSearchContext as source material before answering.',
          'Treat webSearchContext as search results to analyze, not as a final answer. If search was needed but unavailable or empty, say you could not verify current web results.',
