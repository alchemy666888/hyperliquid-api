import { ASSETS, getHyperliquidSnapshot } from './hyperliquid.js';
import { requestAiChat } from './ai-client.js';
import {
  extractWeatherLocation,
  formatWeatherPlace,
  getDefaultWeatherLocation,
  getWeatherSnapshot,
  isWeatherRelatedRequest,
  weatherCodeDescription,
} from './weather.js';
import {
  escapeTelegramHtml,
  formatTelegramLocalDate,
  htmlMessage,
  sanitizeTelegramHtml,
  telegramTableMessage,
  trimTelegramText,
} from './telegram-format.js';
import { searchForContext } from './search.js';
import {
  buildSearchApiParams,
  extractSearchQuery as defaultExtractSearchQuery,
  getSearchExtractionCache,
  isLikelySearchRequest,
} from './intelligence/search-query.js';

const RESEARCH_SEARCH_RESULT_LIMIT = 8;
const MARKET_TOPIC_PATTERN = /\b(?:asset|assets|bitcoin|btc|coin|coins|crypto|cryptocurrency|entry|entries|equity|equities|forex|future|futures|gold|hyperliquid|indicator|indicators|invest|investing|investment|long|market|markets|nasdaq|portfolio|price|prices|rsi|short|stock|stocks|symbol|symbols|ticker|tickers|trade|trades|trading|trend|volume)\b|[$€£¥]\s?\d/i;

function formatMeasurement(value, unit = '', digits = 1) {
  if (value == null || Number.isNaN(Number(value))) return 'n/a';
  const rounded = Number(value).toLocaleString('en-US', { maximumFractionDigits: digits });
  return `${rounded}${unit}`;
}

function formatPercent(value) {
  return value == null || Number.isNaN(Number(value)) ? 'n/a' : `${Math.round(Number(value))}%`;
}

function hasChineseText(value) {
  return /[\u3400-\u9fff]/.test(String(value ?? ''));
}

function formatAssetForPrompt(asset) {
  const indicators = asset.indicators ?? {};
  const macd = indicators.macd ?? {};
  return {
    symbol: asset.symbol,
    price: asset.price,
    regime: asset.regime,
    rsi14: indicators.rsi14,
    adx14: indicators.adx14,
    ema20: indicators.ema20,
    ema50: indicators.ema50,
    macdHistogram: macd.histogram,
    macdHistogramDirection: macd.histogramDirection,
    atr14: indicators.atr14,
    volumeSpikeRatio: indicators.volumeSpikeRatio,
  };
}

function summarizeSnapshot(snapshot) {
  const assets = Array.isArray(snapshot?.assets) ? snapshot.assets : [];
  return {
    timestamp: snapshot?.timestamp,
    interval: snapshot?.interval,
    assets: assets.map(formatAssetForPrompt),
  };
}

function fallbackSnapshotContext(error) {
  return {
    timestamp: new Date().toISOString(),
    interval: 'unknown',
    assets: [],
    warning: `Fresh market snapshot unavailable: ${error.message}`,
  };
}

function fallbackSearchContext(error, query) {
  const message = error && typeof error.message === 'string' ? error.message : String(error);
  return {
    source: 'searchapi-io',
    query,
    timestamp: new Date().toISOString(),
    ok: false,
    error: `SearchApi.io search failed: ${message}`,
    results: [],
  };
}

function skippedSearchContext(query, reason) {
  return {
    source: 'searchapi-io',
    query,
    timestamp: new Date().toISOString(),
    ok: false,
    skipped: true,
    reason,
    results: [],
  };
}

function convertCommonMarkdownToHtml(value) {
  return String(value ?? '')
    .replace(
      /```(?:\w+)?\s*([\s\S]*?)\s*```/g,
      (_match, code) => `<pre>${escapeTelegramHtml(code)}</pre>`,
    )
    .replace(
      /`([^`\n]+)`/g,
      (_match, code) => `<code>${escapeTelegramHtml(code)}</code>`,
    )
    .replace(/(^|[^*])\*\*([^*\n][^*\n]*?)\*\*/g, '$1<b>$2</b>');
}

export function sanitizeTelegramAiHtml(value) {
  return sanitizeTelegramHtml(convertCommonMarkdownToHtml(value));
}

function formatWeatherAnswer(weather, { currentMessage, usedDefaultLocation }) {
  const chinese = hasChineseText(currentMessage);
  const place = formatWeatherPlace(weather.location) || weather.requestedLocation;
  const current = weather.current ?? {};
  const today = weather.today ?? {};
  const currentUnits = weather.currentUnits ?? {};
  const dailyUnits = weather.dailyUnits ?? {};
  const currentDescription = weatherCodeDescription(current.weather_code, chinese);
  const todayDescription = weatherCodeDescription(today.weatherCode, chinese);
  const temperatureUnit = currentUnits.temperature_2m ?? '°C';
  const windUnit = currentUnits.wind_speed_10m ?? 'km/h';
  const dailyTemperatureUnit = dailyUnits.temperature_2m_max ?? '°C';
  const updatedAt = current.time ? formatTelegramLocalDate(current.time) : 'n/a';

  if (chinese) {
    const defaultLine = usedDefaultLocation ? `未指定城市，按默认地点 ${place} 查询。` : `${place} 的天气：`;
    return htmlMessage(sanitizeTelegramAiHtml(trimTelegramText([
      '<b>天气</b>',
      defaultLine,
      `当前：${currentDescription}，${formatMeasurement(current.temperature_2m, temperatureUnit)}，体感 ${formatMeasurement(current.apparent_temperature, temperatureUnit)}，湿度 ${formatPercent(current.relative_humidity_2m)}。`,
      `今日：${todayDescription}，${formatMeasurement(today.temperatureMin, dailyTemperatureUnit)} - ${formatMeasurement(today.temperatureMax, dailyTemperatureUnit)}，最高降雨概率 ${formatPercent(today.precipitationProbabilityMax)}。`,
      `风速：${formatMeasurement(current.wind_speed_10m, windUnit)}。`,
      `更新时间：${updatedAt}`,
    ].join('\n'))));
  }

  const defaultLine = usedDefaultLocation ? `No location was specified, so I used the default location: ${place}.` : `Weather for ${place}:`;
  return htmlMessage(sanitizeTelegramAiHtml(trimTelegramText([
    '<b>Weather</b>',
    defaultLine,
    `Current: ${currentDescription}, ${formatMeasurement(current.temperature_2m, temperatureUnit)}, feels like ${formatMeasurement(current.apparent_temperature, temperatureUnit)}, humidity ${formatPercent(current.relative_humidity_2m)}.`,
    `Today: ${todayDescription}, ${formatMeasurement(today.temperatureMin, dailyTemperatureUnit)} - ${formatMeasurement(today.temperatureMax, dailyTemperatureUnit)}, max rain chance ${formatPercent(today.precipitationProbabilityMax)}.`,
    `Wind: ${formatMeasurement(current.wind_speed_10m, windUnit)}.`,
    `Updated: ${updatedAt}`,
  ].join('\n'))));
}

function weatherUnavailableAnswer(error, currentMessage) {
  if (hasChineseText(currentMessage)) {
    return telegramTableMessage('天气查询不可用', [
      ['状态', error.message ?? '暂时无法获取天气。'],
      ['下一步', '请带上城市再试一次，例如：吉隆坡今天天气如何。'],
    ]);
  }

  return telegramTableMessage('Weather unavailable', [
    ['Status', error.message ?? 'Weather could not be fetched right now.'],
    ['Next step', 'Try again with a city, for example: weather in Kuala Lumpur today.'],
  ]);
}

function assetAliases(asset) {
  const symbol = String(asset?.symbol ?? asset?.label ?? '').toUpperCase();
  const coin = String(asset?.coin ?? '').replace(/^xyz:/i, '').toUpperCase();
  const base = symbol.endsWith('USDT') ? symbol.slice(0, -4) : symbol;
  return [symbol, coin, base].filter(Boolean);
}

function isMarketRelatedRequest(message) {
  const text = String(message ?? '').trim();
  if (!text) return false;
  if (MARKET_TOPIC_PATTERN.test(text)) return true;

  const normalizedWords = text.toUpperCase().match(/[A-Z0-9]+/g) ?? [];
  const words = new Set(normalizedWords);
  return ASSETS.some(asset =>
    assetAliases(asset).some(alias => words.has(alias))
  );
}

export function isWebSearchRelatedRequest(message, { marketRelatedRequest = false } = {}) {
  return isLikelySearchRequest(message, { marketRelatedRequest });
}

async function resolveSearchContext({
  currentMessage,
  getSearch,
  aiJson,
  extractSearchQuery = defaultExtractSearchQuery,
  extractionCache,
} = {}) {
  const cache = extractionCache === undefined ? getSearchExtractionCache() : extractionCache;
  const extracted = await extractSearchQuery({
    message: currentMessage,
    aiJson,
    cache,
  });

  if (!extracted.needs_search) {
    return {
      extracted,
      webSearchRelatedRequest: false,
      webSearchContext: skippedSearchContext(extracted.q || currentMessage, 'needs_search=false'),
    };
  }

  const params = buildSearchApiParams(extracted);
  if (!params) {
    return {
      extracted,
      webSearchRelatedRequest: false,
      webSearchContext: skippedSearchContext(extracted.q || currentMessage, 'empty-search-query'),
    };
  }

  try {
    return {
      extracted,
      webSearchRelatedRequest: true,
      webSearchContext: await getSearch({
        params,
        limit: RESEARCH_SEARCH_RESULT_LIMIT,
      }),
    };
  } catch (error) {
    return {
      extracted,
      webSearchRelatedRequest: true,
      webSearchContext: fallbackSearchContext(error, params.q),
    };
  }
}

export async function answerStatelessAiChat({
  message,
  getSnapshot = getHyperliquidSnapshot,
  getWeather = getWeatherSnapshot,
  getSearch = searchForContext,
  aiJson,
  extractSearchQuery = defaultExtractSearchQuery,
  extractionCache,
  deepSeekChat,
  aiChat = deepSeekChat ?? requestAiChat,
} = {}) {
  const currentMessage = String(message ?? '').trim();
  if (!currentMessage) {
    return telegramTableMessage('AI chat', [
      ['Status', 'Send a message and I will answer using the current request only.'],
    ]);
  }

  if (isWeatherRelatedRequest(currentMessage)) {
    const explicitLocation = extractWeatherLocation(currentMessage);
    const location = explicitLocation || getDefaultWeatherLocation();

    try {
      const weather = await getWeather({ location });
      return formatWeatherAnswer(weather, {
        currentMessage,
        usedDefaultLocation: !explicitLocation,
      });
    } catch (error) {
      return weatherUnavailableAnswer(error, currentMessage);
    }
  }

  const marketRelatedRequest = isMarketRelatedRequest(currentMessage);
  let snapshotContext = null;
  if (marketRelatedRequest) {
    try {
      snapshotContext = summarizeSnapshot(await getSnapshot());
    } catch (error) {
      snapshotContext = fallbackSnapshotContext(error);
    }
  }
  let searchExtraction = null;
  let webSearchContext = skippedSearchContext('', 'not-search-related');
  let webSearchRelatedRequest = isWebSearchRelatedRequest(currentMessage, { marketRelatedRequest });
  if (webSearchRelatedRequest) {
    const resolvedSearch = await resolveSearchContext({
      currentMessage,
      getSearch,
      aiJson,
      extractSearchQuery,
      extractionCache,
    });
    searchExtraction = resolvedSearch.extracted;
    webSearchContext = resolvedSearch.webSearchContext;
    webSearchRelatedRequest = resolvedSearch.webSearchRelatedRequest;
  }

  const result = await aiChat({
    messages: [
      {
        role: 'system',
        content: [
          'You are a stateless AI assistant inside a Telegram bot.',
          'Answer the user naturally without requiring slash commands, including friendly daily-life topics such as routines, food, travel, hobbies, planning, and casual conversation.',
          'Use only the current user request, webSearchContext, and marketContext when marketContext is present in this prompt.',
          'Do not refer to or infer previous conversation history, even if history is stored elsewhere.',
          'Use the market context only when it is present and the request is about markets, assets, prices, trading, or investing; do not force market commentary into ordinary daily-life replies.',
          'For real-time prices, indicators, and tracked asset snapshots, use the Hyperliquid marketContext as the source of truth.',
          'When webSearchRelatedRequest is true, use webSearchContext as source material before answering.',
          'Treat webSearchContext as search results to analyze, not as a final answer. If webSearchRelatedRequest is true and webSearchContext is unavailable or empty, say when you could not verify current web results.',
          'Be concise enough for Telegram. If discussing markets, trades, or investing, be clear that it is informational and not financial advice.',
          'Format replies as Telegram-compatible HTML, not Markdown. Use only simple tags such as <b>, <i>, <u>, <code>, and <pre> when formatting helps. Do not use Markdown syntax like **bold**, _italic_, # headings, or fenced code blocks.',
          'The bot can also handle slash commands: /prices, /asset SYMBOL, /rsh QUERY, /treealert, /condition SYMBOL, /alerts, /clearalerts.',
        ].join(' '),
      },
      {
        role: 'user',
        content: JSON.stringify({
          currentRequest: currentMessage,
          marketRelatedRequest,
          marketContext: snapshotContext,
          webSearchRelatedRequest,
          searchExtraction,
          webSearchContext,
        }),
      },
    ],
  });

  if (!result.ok) {
    const missing = Array.isArray(result.missing) && result.missing.length
      ? result.missing.join(', ')
      : 'AI_MODEL_PROVIDER, CLAUDE_API_KEY, or DEEPSEEK_API_KEY';
    return telegramTableMessage('AI chat unavailable', [
      ['Status', result.error ?? 'AI could not answer right now.'],
      ['Next step', `Check ${missing} configuration or try again later.`],
    ]);
  }

  const text = trimTelegramText(result.text);

  return htmlMessage(sanitizeTelegramAiHtml(text));
}

export async function answerResearchChat({
  message,
  getSnapshot = getHyperliquidSnapshot,
  getSearch = searchForContext,
  aiJson,
  extractSearchQuery = defaultExtractSearchQuery,
  extractionCache,
  deepSeekChat,
  aiChat = deepSeekChat ?? requestAiChat,
} = {}) {
  const currentMessage = String(message ?? '').trim();
  if (!currentMessage) {
    return telegramTableMessage('Research usage', [
      ['Usage', '/rsh BTC latest catalysts'],
      ['Scope', 'Fresh market, trading, macro, equity, and crypto news research.'],
    ]);
  }

  const marketRelatedRequest = isMarketRelatedRequest(currentMessage);
  let snapshotContext = null;
  if (marketRelatedRequest) {
    try {
      snapshotContext = summarizeSnapshot(await getSnapshot());
    } catch (error) {
      snapshotContext = fallbackSnapshotContext(error);
    }
  }

  const {
    extracted: searchExtraction,
    webSearchRelatedRequest,
    webSearchContext,
  } = await resolveSearchContext({
    currentMessage,
    getSearch,
    aiJson,
    extractSearchQuery,
    extractionCache,
  });

  const result = await aiChat({
    messages: [
      {
        role: 'system',
        content: [
          'You are a Telegram research analyst for market, trading, macro, equity, and crypto news.',
          'Answer the user in the same language they used.',
          'Use the original currentRequest to preserve intent, then use freshArticles from webSearchContext as the current source material.',
          'Do not treat the cleaned search query as the user intent; it is only a retrieval query.',
          'If webSearchRelatedRequest is false, reply naturally without pretending that current web research was performed.',
          'If webSearchRelatedRequest is true but results are empty or unavailable, say that current news could not be verified.',
          'For real-time tracked market prices and indicators, use marketContext when present.',
          'Be concise enough for Telegram and include informational-only wording for trading or investing discussion.',
          'Format replies as Telegram-compatible HTML, not Markdown. Use only <b>, <i>, <u>, <code>, and <pre> when helpful.',
        ].join(' '),
      },
      {
        role: 'user',
        content: JSON.stringify({
          currentRequest: currentMessage,
          marketRelatedRequest,
          marketContext: snapshotContext,
          webSearchRelatedRequest,
          searchExtraction,
          freshArticles: webSearchContext?.results ?? [],
          webSearchContext,
        }),
      },
    ],
  });

  if (!result.ok) {
    const missing = Array.isArray(result.missing) && result.missing.length
      ? result.missing.join(', ')
      : 'AI_MODEL_PROVIDER, CLAUDE_API_KEY, or DEEPSEEK_API_KEY';
    return telegramTableMessage('Research unavailable', [
      ['Status', result.error ?? 'AI could not answer right now.'],
      ['Next step', `Check ${missing} configuration or try again later.`],
    ]);
  }

  return htmlMessage(sanitizeTelegramAiHtml(trimTelegramText(result.text)));
}
