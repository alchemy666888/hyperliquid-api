...
export async function answerStatelessAiChat({
  ...
  try {
    webSearchContext = await getSearch({ query: currentMessage });
  } catch (error) {
    webSearchContext = fallbackSearchContext(error, currentMessage);
  }
  ...
  const webSearchRelatedRequest = true;
    ...
    messages: [
      {
        ...
        content: [
          ...
          'For every answer, use webSearchContext as source material before answering.',
          'Treat webSearchContext as search results to analyze, not as a final answer. If webSearchContext is unavailable or empty, say when you could not verify current web results.',
