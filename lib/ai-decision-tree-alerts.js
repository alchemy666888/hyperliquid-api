...
function deterministicClassification({ asset, currentPrice, indicators, activeRules }) {
  const matchedRules = activeRules.filter(rule => matchesDecisionTreeRule(rule, currentPrice, indicators));
...
export async function classifyAssetDecisionTreeCondition({
  ...
  const fallback = deterministicClassification({
    asset,
    currentPrice: price,
    indicators: indicators ?? asset?.indicators ?? {},
    activeRules: rules,
  });
