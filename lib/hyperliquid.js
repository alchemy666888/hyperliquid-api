import { getPostgresStatus, saveHyperliquidSnapshot } from './postgres.js';
  ...
  if (!postgresStatus.configured) {
    return {
      ...
      persistence: {
        enabled: false,
        saved: false,
        missing: postgresStatus.missing,
      },
    };
  }

  try {
    const persisted = await saveHyperliquidSnapshot(snapshot);
    return {
      ...snapshot,
      persistence: {
        enabled: true,
        saved: Boolean(persisted),
        id: persisted?.id,
        capturedAt: persisted?.capturedAt,
      },
    };
  } catch (error) {
    console.error('postgres snapshot save error:', error);
    return {
      ...snapshot,
      persistence: {
        enabled: true,
        saved: false,
        error: 'PostgreSQL persistence failed',
      },
    };
  }
}
