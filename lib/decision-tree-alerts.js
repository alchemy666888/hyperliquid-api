...
import { getAiConfig, requestAiJson } from './ai-client.js';

const ARROW_PATTERN = /^(?:→|->|=>)\s*(.+)$/;
const SAME_LINE_PATTERN = /^(.+\?)\s*(?:→|->|=>)\s*(.+)$/;
const PRICE_PATTERN = String.raw`\$?\s*([0-9][0-9,]*(?:\.\d+)?)`;
export const SUPPORTED_TECHNICAL_KINDS = new Set([
  'rsi_below',
  'rsi_above',
  'macd_cross_up',
  'macd_cross_down',
...
export const SUPPORTED_PRICE_KINDS = new Set(['above', 'below', 'between']);
export const SUPPORTED_CONDITION_KINDS = new Set([
...
export function normalizeConditionKind(value) {
  const kind = String(value ?? '').trim().toLowerCase();
  return SUPPORTED_CONDITION_KINDS.has(kind) ? kind : '';
}
...
function normalizeAiRule(rule, assets) {
  ...
  if (conditionKind === 'above' && abovePrice == null) return null;
  if (conditionKind === 'below' && belowPrice == null) return null;
  ...
  return {
    ...
    conditionText,
    conditionKind,
    lowerPrice: conditionKind === 'above' ? abovePrice : (range.lowerPrice ?? null),
    upperPrice: conditionKind === 'below' ? belowPrice : (range.upperPrice ?? null),
    actionText,
  };
}
...
function macdCrossedUp(indicators) {
  const macd = indicators?.macd ?? {};
  const line = finiteNumber(macd.macd);
  const signal = finiteNumber(macd.signal);
  const histogram = finiteNumber(macd.histogram);
  const direction = String(macd.histogramDirection ?? '').toLowerCase();

  // Cross-up proxy from the 4H snapshot: MACD data is present, histogram is
  // non-negative, and the histogram is increasing versus the prior candle.
  return line != null
    && signal != null
    && histogram != null
    && histogram >= 0
...
function macdCrossedDown(indicators) {
  const macd = indicators?.macd ?? {};
  const line = finiteNumber(macd.macd);
  const signal = finiteNumber(macd.signal);
  const histogram = finiteNumber(macd.histogram);
  ...
  // Cross-down proxy from the 4H snapshot: MACD data is present, histogram is
  // non-positive, and the histogram is decreasing versus the prior candle.
  return line != null
    && signal != null
    && histogram != null
    && histogram <= 0
...
export function matchesDecisionTreeRule(rule, price, indicators = null) {
  ...
  if (rule.conditionKind === 'rsi_below') {
    const rsi14 = finiteNumber(indicators?.rsi14);
    const threshold = finiteNumber(rule.indicatorParams?.threshold);
    return rsi14 != null && threshold != null && rsi14 <= threshold;
  }

  if (rule.conditionKind === 'rsi_above') {
    const rsi14 = finiteNumber(indicators?.rsi14);
    const threshold = finiteNumber(rule.indicatorParams?.threshold);
    return rsi14 != null && threshold != null && rsi14 >= threshold;
  }

  if (rule.conditionKind === 'macd_cross_up') {
    return macdCrossedUp(indicators);
  }

  if (rule.conditionKind === 'macd_cross_down') {
    return macdCrossedDown(indicators);
  }

  if (rule.conditionKind === 'ema_cross_up') {
  ...
  if (rule.conditionKind === 'ema_cross_down') {
    const ema20 = finiteNumber(indicators?.ema20);
    const ema50 = finiteNumber(indicators?.ema50);
    return ema20 != null && ema50 != null && ema20 <= ema50;
  }
...
function deterministicAlertParseFallback(input, assets, extra) {
...
export async function parseDecisionTreeAlertTextWithAi(input, {
  assets = [],
  deepSeekRequest,
  aiRequest = deepSeekRequest ?? requestAiJson,
} = {}) {
  const config = getAiConfig();
  if (!config.configured) {
    return deterministicAlertParseFallback(input, assets, {
      aiAttempted: false,
      aiNeeded: true,
      aiUnavailable: true,
      aiMessage: `AI analysis needs ${config.missing.join(', ')}. Saved deterministic rules only if any were recognized.`,
    });
  }
    ...
    messages: [
      {
        role: 'system',
        content: [
          'Analyze decision-tree trading alert text and extract trigger-ready alerts as strict JSON only.',
          ...
          'Use above for breakouts/reclaims/sustains above a trigger, below for breakdowns/fails below a trigger, and between for holds/rejects/tests inside a range.',
  ...
  if (!response.ok) {
    return deterministicAlertParseFallback(input, assets, {
      aiAttempted: true,
      aiNeeded: true,
      aiMessage: response.error || 'AI analysis failed.',
    });
  }
  ...
  if (!rules.length) {
    return deterministicAlertParseFallback(input, assets, {
      aiAttempted: true,
      aiNeeded: true,
      aiMessage: 'AI analysis returned no valid trigger rules.',
    });
  }
