...
export function postgresSslFromConnectionOptions(options) {
...
export function buildTelegramChatHistories(rows = []) {
  const chats = [];
  ...
  for (const row of rows) {
    ...
    if (!chat) {
      chat = {
        chatId,
        latestMessageAt: row.latest_message_at?.toISOString?.() ?? row.latest_message_at,
    ...
    chat.messages.push(normalizeTelegramChatMessageRow(row));
  }
...
export async function listTelegramChatHistories({
  chatLimit = 50,
  historyLimit = 20,
} = {}) {
  const ready = await ensureSchema();
  ...
  const result = await pool.query(
    `
      WITH selected_chats AS (
        ...
        GROUP BY chat_id
        ORDER BY latest_message_at DESC, chat_id ASC
        LIMIT $1
      ),
      ranked_messages AS (
        SELECT
        ...
        JOIN selected_chats c ON c.chat_id = m.chat_id
      )
      SELECT *
      FROM ranked_messages
      WHERE message_rank <= $2
      ORDER BY latest_message_at DESC, chat_id ASC, created_at ASC, id ASC
    `,
    [chatLimit, historyLimit],
  );

  return buildTelegramChatHistories(result.rows);
}
