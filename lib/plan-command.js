...
import {
  ...
  trimTelegramText,
} from './conversational-ai.js';
import { getPostgresStatus, saveDecisionTreeAlerts } from './postgres.js';
...
function formatExecutionSide(label, side) {
  ...
  return [
    ...
    `Entries: ${escapeTelegramHtml(formatList(side.entries))}`,
    `Stop/invalidation: ${escapeTelegramHtml(side.stop ?? 'n/a')}`,
    `Targets: ${escapeTelegramHtml(formatList(side.targets))}`,
    side.rationale ? `Rationale: ${escapeTelegramHtml(side.rationale)}` : '',
  ].filter(Boolean).join('\n');
}
...
export function formatPlanReply({
  ...
  const title = `SwingScope plan: ${symbol}`;
  ...
  const noteText = allNotes.length
    ? `\n\n<b>Notes</b>\n${allNotes.map(note => escapeTelegramHtml(note)).join('\n')}`
    : '';
  ...
  const raw = [
    ...
    `<b>Execution</b>\n${execution}`,
    `<b>Status</b>\n${escapeTelegramHtml(status)}${noteText}`,
  ].join('\n\n');

  return htmlMessage(trimTelegramText(sanitizeTelegramAiHtml(raw)));
}
...
function planUnavailableMessage(result) {
  const missing = Array.isArray(result?.missing) && result.missing.length
    ? result.missing.join(', ')
    : 'AI_MODEL_PROVIDER, CLAUDE_API_KEY, or DEEPSEEK_API_KEY';
  return telegramTableMessage('Plan unavailable', [
    ['Status', result?.error ?? 'AI could not assemble the plan right now.'],
    ['Next step', `Check ${missing} configuration or try again later.`],
  ]);
}

function executionBlockText(plan = {}) {
  const sections = [];
  if (plan.long) {
    sections.push([
      'Long',
...
export async function runPlanCommand({
  ...
  try {
    ...
    const researchRunner = deps.runResearchStages ?? runResearchStages;
    ...
    if (!research?.ok && research?.status === 'ai-unavailable') {
      return planUnavailableMessage(research);
    }
    ...
    if (!research?.ok) {
      const pipeline = {
        ...
        horizon: parsed.horizon,
        plan: {
          analysisSummary: `The staged research workflow could not complete cleanly: ${research?.error ?? 'unknown error'}.`,
          conditions: [],
          degraded: true,
        },
        notes: research?.notes ?? [],
      };
      return formatPlanReply({ pipeline, notes: ['Workflow degraded before plan assembly.'] });
    }

    const assembled = await planAssembler({
      symbol,
      resolvedSymbol: resolved.resolvedSymbol,
    ...
    const plan = assembled?.plan ?? { analysisSummary: research.inference, conditions: [] };
    if (!assembled?.ok && assembled?.error) {
      notes.push(`Plan assembly degraded: ${assembled.error}`);
    }
    ...
    if (resolved.alertable) {
      const normalized = normalizer(plan.conditions ?? [], {
        assets,
      ...
      if (!postgresStatus().configured) {
        ...
        try {
          const saved = await saveAlerts({
            chatId,
            rawTree: executionBlockText(plan),
            rules,
          });
    ...
    return formatPlanReply({
      pipeline: {
        symbol: String(symbolInput ?? args?.[0] ?? '').trim() || 'symbol',
        resolvedSymbol: String(symbolInput ?? args?.[0] ?? '').trim() || 'symbol',
        alertable: false,
        plan: {
          analysisSummary: `The plan workflow hit an unexpected error: ${error.message}`,
          conditions: [],
          degraded: true,
        },
      },
      notes: ['Workflow degraded after an unexpected error.'],
    });
  }
}
