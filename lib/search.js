...
const TINYFISH_PROVIDER = 'TINYFISH';
...
function sanitizedSearchApiUrl(url) {
  ...
  if (safeUrl.searchParams.has('query')) safeUrl.searchParams.set('query', '[redacted-query]');
  return safeUrl.href;
}
...
function validateTinyfishConfig(config) {
  const missing = [];
  if (!String(config?.apiKey ?? '').trim()) missing.push('TINYFISH_API_KEY');
  return missing;
}
...
function normalizeSearchProvider(value) {
  ...
  if (provider === 'TINYFISH' || provider === 'TINYFISH_AI' || provider === 'SEARCH_TINYFISH') return TINYFISH_PROVIDER;
...
export function getSearchConfig(env = process.env) {
  ...
  if (provider === TINYFISH_PROVIDER) {
    const tinyfishApiKey = readEnv(env, 'TINYFISH_API_KEY') || readEnv(env, 'TINYFISH_SEARCH_API_KEY');
    const tinyfishMissing = validateTinyfishConfig({ apiKey: tinyfishApiKey });
    return {
      provider,
      configured: tinyfishMissing.length === 0,
      missing: tinyfishMissing,
...
export function createTinyfishSearchTool(config = getSearchConfig(), { fetchImpl = fetch } = {}) {
  return {
    name: 'tinyfish',
    description: 'Searches Tinyfish for current web information.',
    async invoke(input) {
      const missing = validateTinyfishConfig(config);
      const params = normalizeSearchApiParams(input, config);
      if (missing.length) {
        console.warn('Tinyfish search request skipped: missing configuration', {
          missing,
          apiKeyConfigured: Boolean(String(config?.apiKey ?? '').trim()),
          resultLimit: config?.resultLimit,
          queryLength: params.q.length,
        });
        throw new Error(`Tinyfish search is not configured. Missing ${missing.join(', ')}.`);
      }

      if (!params.q) {
        throw new Error('Tinyfish search query is required.');
      }

      const url = new URL(TINYFISH_URL);
      url.searchParams.set('query', params.q);
      for (const [key, value] of Object.entries(params)) {
        if (value && key !== 'q' && key !== 'engine') url.searchParams.set(key, value);
      }
      ...
      if (!response.ok) {
        ...
        console.warn('Tinyfish search request failed', {
          status: response.status,
          statusText: response.statusText ?? '',
          url: sanitizedSearchApiUrl(url),
          apiKeyConfigured: Boolean(config.apiKey),
          requestHasApiKeyHeader: Boolean(config.apiKey),
          requestHasQueryParam: Boolean(url.searchParams.get('query')),
          resultLimit: config.resultLimit,
          queryLength: params.q.length,
          ...details,
        });
        throw new Error(`Tinyfish search returned HTTP ${response.status}.${searchApiErrorSummary(details)}`);
      }
...
export function createSearchTool(config = getSearchConfig(), { fetchImpl = fetch } = {}) {
  if (config?.provider === TINYFISH_PROVIDER) {
    return createTinyfishSearchTool(config, { fetchImpl });
  }
...
export async function searchForContext({
  ...
  const source = config.provider === WEBSEARCH_DEEPSEEK_PROVIDER
    ? 'websearch-deepseek-mcp'
    : config.provider === TINYFISH_PROVIDER
      ? 'tinyfish'
      : 'searchapi-io';
  const base = {
    source,
  ...
  if (!searchTool && !config.configured) {
    return {
      ...
      error: config.provider === WEBSEARCH_DEEPSEEK_PROVIDER
        ...
        : config.provider === TINYFISH_PROVIDER
          ? 'Tinyfish search is not configured.'
