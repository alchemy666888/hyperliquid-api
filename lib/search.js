...
const DEFAULT_SEARCHAPI_ENGINE = 'google';
...
function mapSearchApiOrganicResults(items) {
...
export function createSearchTool(config = getSearchConfig(), { fetchImpl = fetch } = {}) {
  return {
    ...
    async invoke(input) {
      ...
      url.searchParams.set('engine', config.engine || DEFAULT_SEARCHAPI_ENGINE);
      ...
      const payload = await response.json();
      return JSON.stringify(mapSearchApiOrganicResults(payload?.organic_results));
    },
  };
}
...
export async function searchForContext({
  ...
  if (!searchTool && !config.configured) {
    return {
      ...base,
      ok: false,
      error: 'SearchApi.io search is not configured.',
      missing: config.missing,
      results: [],
    };
  }
  ...
  try {
    const tool = searchTool ?? createSearchTool(config);
