...
`/prices` returns all tracked prices and regimes. `/asset <symbol>` returns a detailed 4H indicator snapshot for one asset. `/condition <symbol>` fetches the current asset state and asks DeepSeek to classify it against the saved decision tree for the current Telegram chat.
